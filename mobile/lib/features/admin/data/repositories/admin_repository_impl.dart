...
class AdminRepositoryImpl implements AdminRepository {
  ...
  Future<void> toggleUserStatus(String userId, String statut) async {
    try {
      ...
      final response =
          await _dio.patch('/users/$userId/statut', data: {'statut': statut});
    ...
    try {
      ...
      if (data is Map<String, dynamic> && data.containsKey('data')) {
        ...
        final pending = allOrders
            .where((c) => c.statut == 'EN_ATTENTE' || c.statut == 'VALIDEE')
            .toList();
  ...
  Future<List<UserDto>> getLivreurs() async {
    try {
      ...
      if (data is Map<String, dynamic> && data.containsKey('data')) {
        final list = data['data'] as List;
        // ✅ Construction manuelle pour éviter le parsing de 'role' (qui n'existe pas)
        final livreurs = list.map((e) {
          final item = e as Map<String, dynamic>;
          return UserDto(
            id: item['id'] ?? '',
            nom: item['nom'] ?? '',
            prenom: item['prenom'] ?? '',
            email: '', // Pas d'email dans la réponse
            role:
                'LIVREUR', // On force le rôle (car c'est une liste de livreurs)
        ...
        print('✅ [AdminRepo] ${livreurs.length} livreurs trouvés');
        return livreurs;
      } else {
        throw Exception('Format de réponse inattendu');
      }
  ...
  Future<void> assignerLivraison(String commandeId, String livreurId) async {
    try {
      print(
          '🚚 [AdminRepo] Assignation commande $commandeId au livreur $livreurId');
  ...
  @override
  Future<void> createUserByAdmin(String nom, String prenom, String email,
      String telephone, String password, String role) async {
    try {
